...
// a window listener is used to save data before closing the window, or to exit the program when the last window closes
	...
	project.pack();//all contents are bounded in window
	project.setVisible(true);//makes frame appear on screen
    }
    ...
    //changing state acc to the action event
	if (buttonIdentifier.equals("addVertex")) {
        ...
        {
            result+=mst.get(i).weight;
        }
        System.out.println("result weight of mst is :"+result);
        canvas.repaint();
    }
        ...
        if (input.length() <= 7) {
            double double1;
            //handling error for entered weight of edge
            try {
    ...
    public int onVertex(Point point) {
        // if the point of the event occured on a vertex, return the index of that vertex in vertices
        // if not, return -1
        int x = -1;
        ...
        for (int i = 0; i < vertices.size(); ++i) {
                ...
                //as node radus is 8px
            if (point.distance(vertex.p) <= 8.0) {
                x = i;//index of current vertex
                vertex.hovered = true;
                break;
            }
    ...
    public int onEdge(Point point) {
        ...
        if (clickedEdgeIndex > -1) {
        		...
        		double distanceFromEdge =
                        ...
                        //function used to calculate distance of a point from a line
